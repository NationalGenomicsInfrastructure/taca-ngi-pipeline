...
class TestTsvGenerator(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        couch_doc = {
            ...
            "samples": {
                "P12345_1001": {"library_prep": {"A": {"sequenced_fc": "ABC123"}}}
            },
    ...
    @patch("taca_ngi_pipeline.utils.ena_tsv_generator.ProjectSummaryConnection")
    @patch("taca_ngi_pipeline.utils.ena_tsv_generator.GenericFlowcellRunConnection")
    def test_validation_error_log_file_created(
        self, mock_fc_conn, mock_proj_conn, mock_validate
    ):
        """Test that a dedicated validation error log file is created."""
        project_doc = {
            "project_id": self.pid,
            "staged_files": {
                "P12345_1001": {
        ...
        self.assertTrue(os.path.exists(gen.error_log_file))
        with open(gen.error_log_file, "r") as error_log_handle:
            error_log_contents = error_log_handle.read()
        self.assertIn(
            "Row 2: Missing required field: library_strategy", error_log_contents
        )
...
def test_generate_and_validate_tsv_file(self, mock_fc_conn, mock_proj_conn):
    ...
    project_doc = {
        ...
        "samples": {"P12345_1001": {"customer_name": "Sample_001"}},
        "open_date": "2024-01-01",
    }
    mock_proj_conn.return_value.get_entry.return_value = project_doc
    mock_fc_conn.return_value.get_project_flowcell.return_value = {}
    ...
    self.assertEqual(
        tsv_file_path, os.path.join(self.outdir, f"{self.pid}_submission.tsv")
    )
    ...
    def test_validate_tsv_file(self, mock_fc_conn, mock_proj_conn, mock_validate):
        ...
        gen = tsv_generator(
            self.pid, outdir=self.outdir, LOG=self.log, db_conf=self.db_conf
        )
        tsv_file_path = gen.generate_tsv_file()
        gen.validate_tsv_file(tsv_file_path)
    ...
    @patch("taca_ngi_pipeline.utils.ena_tsv_generator.ProjectSummaryConnection")
    @patch("taca_ngi_pipeline.utils.ena_tsv_generator.GenericFlowcellRunConnection")
    def test_validate_tsv_file_with_errors(
        self, mock_fc_conn, mock_proj_conn, mock_validate
    ):
